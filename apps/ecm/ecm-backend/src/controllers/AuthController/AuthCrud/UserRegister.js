...
const registerUser = async (req, res) => {
  try {
    ...
    const {
      ...
      phone,
      password,
      role,  // This is optional, will use default if not provided
      isAdmin,
      enabled,
      mobile_access,
    } = req.body;
    ...
    // 1️⃣ Check if role is provided, if not, get the default role from the Role model
    let userRole;
    if (role) {
      // If role is provided, use it
      userRole = role;
    } else {
      // If no role is provided, fetch the default role from the Role model
      const defaultRole = await Role.findOne({ value: 0 });  // Default role (value: 0)
      ...
      userRole = defaultRole._id;  // Assign ObjectId of the default role
    }
    ...
    const newUser = new User({
      ...
      role: userRole,  // Assign the role (either passed or default)
